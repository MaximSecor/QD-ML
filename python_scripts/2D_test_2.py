...
def generate_pot():
    ...
    potential_1 = 0.5*mass*(KX_1**2)*(X-X0)**2 + 0.5*mass*(KY_1**2)*(Y-Y0)**2 
    potential_2 = 0.5*mass*(KX_2**2)*(X+X0)**2 + 0.5*mass*(KY_2**2)*(Y+Y0)**2 + dE
...
CS = plt.contour(X, Y, test_pot_2*627,levels = [2,10,25,50,100,200,400,600], colors='w', linestyles="solid")
...
for i in range(250):
    ...
    plt.contourf(xlist,xlist,timeseries_true[i].reshape(grid_size,grid_size).T,levels=[0.0,0.03,0.05,0.075,0.1,0.2,0.3,0.4])
    plt.text(1.1, 1.25, str(str(int(tstep))+" fs"), color="w", fontsize=12)
...
for i in range(250):
    ...
    plt.contourf(xlist,xlist,timeseries_pred[i].reshape(grid_size,grid_size).T,levels=[0.0,0.03,0.05,0.075,0.1,0.2,0.3,0.4])
    plt.text(1.1, 1.25, str(str(int(tstep))+" fs"), color="w", fontsize=12)
...
for i in range(250):
    ...
    plt.contourf(xlist,xlist,(np.abs(timeseries_pred[i]-timeseries_true[i])).reshape(grid_size,grid_size).T,levels=[0.0,0.03,0.05,0.075,0.1,0.2,0.3,0.4])
    plt.text(1.1, 1.25, str(str(int(tstep))+" fs"), color="w", fontsize=12)
...
def generate_pot(length):
    ...
    R_freq = ((200*np.random.random()+100)/(627*350))*10
    R_eq = (1*np.random.random())*1.8897
    R_0 = (2.0*R_eq*np.random.random())
    
    f0 = 55.734
    mu = 0.2652
    lam = (10*np.random.random())
    X_freq = np.sqrt(f0/mu)*0.001*100
    X_eq = (10*np.random.random()-10)
    ...
    for t in range(length):
        ...
        potential_1 = 0.5*mass*(KX_1**2)*(X-(0.5*R_coord))**2 + 0.5*mass*(KY_1**2)*Y**2 
        potential_2 = 0.5*mass*(KX_2**2)*(X+(0.5*R_coord))**2 + 0.5*mass*(KY_2**2)*Y**2 + X_coord*(1/627)
        couplings = np.full((grid_size,grid_size),10)*(1/627)
...
for i in range(10):
    plt.contourf(xlist,xlist,pot_traj[i*100])
    plt.show()

# plt.contourf(xlist,xlist,pot_traj[0])
# plt.show()

# plt.contourf(xlist,xlist,pot_traj[999])
# plt.show()
