...
for q in range(1):
    ...
    for f in range(grid_factors):
        ...
        for t in range(time_steps):
            ...
            overlap_agg_err[f,i,t] += (get_den(np.sum(state_sparse*np.conj(true_state_traj[t][::factor])*np.sqrt(factor))))
            density_agg_err[f,i,t] += np.sum(np.abs((get_den(state_sparse)-get_den(true_state_traj[t][::factor])*factor)))   
            sparse_state_traj.append(state_sparse)
        sparse_state_traj = np.array(sparse_state_traj)
...
for i in range(100):
    ...
    plt.ylim(-5,50)
    plt.plot(xlist,get_den(true_state_traj[tstep])*2500,'b',xlist,ground_state_PES_1*627,'k')
    plt.plot(xlist[::factor],get_den(sparse_state_traj[tstep])*2500/factor,'r')
